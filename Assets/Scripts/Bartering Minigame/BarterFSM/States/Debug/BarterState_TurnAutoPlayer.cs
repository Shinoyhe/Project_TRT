...
public class BarterState_TurnAutoPlayer : BarterBaseState
{
    ...
    public BarterState_TurnAutoPlayer(string stateName, BarterStateMachine machine) : base(stateName, machine) {}
    ...
    public override void Enter(BarterBaseState previousState)
    {
        ...
        if (cardsToPlay > handList.Count) {
            Debug.LogError("BarterState_TurnAutoPlayer Error: Enter failed. The BarterDirector "
                        + $"wants the player CardUser to play more cards ({cardsToPlay}) than it "
                        + $"has in its hand ({handList.Count})");
            return;
        }
        ...
        _machine.Dir.SetPlayerCards(playedCards);

        Debug.Log($"Player submitted: {string.Join(", ", playedCards.Select(x => x.Id))}");
